...
def init(path=None):
    ...
    CREATE TABLE IF NOT EXISTS sim(
      ...
      cb_limit REAL DEFAULT 0.03, exposure_mult REAL DEFAULT 1.0, cb_mode TEXT DEFAULT 'block');
    ...
    for col, dflt in (("cb_limit", "0.03"), ("exposure_mult", "1.0")):   # 기존 DB 마이그레이션(REAL)
        try:
            ...
            pass   # 이미 있음
    try:
        c.execute("ALTER TABLE sim ADD COLUMN cb_mode TEXT DEFAULT 'block'")   # 청산 방식(block|liq)
    except sqlite3.OperationalError:
        pass
    c.commit(); c.close()
